...
/* ===============================
   RELATÓRIOS – UI + RENDERIZAÇÃO
   =============================== */

(function(){
  // Estado local dos relatórios
  const R = {
    tab: 'fluxo',
    periodo: '6m',
    tipo: 'todos',
    categoria: 'todas',
    charts: {} // refs de Chart.js por id
  };

  // Helpers
  const qs  = sel => document.querySelector(sel);
  const qsa = sel => Array.from(document.querySelectorAll(sel));
  const brl = v => (v==null?0:Number(v)).toLocaleString('pt-BR',{style:'currency',currency:'BRL'});
  const ym  = d => String(d).slice(0,7);
  const thisMonthYM = () => (window.S && S.month) || new Date().toISOString().slice(0,7);

  function rangeMeses(periodo){
    const base = new Date((thisMonthYM()+'-01').replace(/-/g,'/'));
    const out = [];
    let n = 6;
    if (periodo==='3m') n = 3;
    else if (periodo==='6m') n = 6;
    else if (periodo==='12m') n = 12;
    else if (periodo==='ytd') {
      const now = new Date();
  ...
  function filtrarTx(){
    const tx = (S.tx||[]).filter(t => t.data);
    const meses = rangeMeses(R.periodo);
    return tx.filter(t=>{
      const okMes = meses.includes(String(t.data).slice(0,7));
      const okTipo = (R.tipo==='todos') ? true : (t.tipo===R.tipo);
      const okCat = (R.categoria==='todas') ? true : (t.categoria===R.categoria);
...
R.charts[id] = null;
  ...
  function renderFluxo(){
    const ctx = qs('#chartFluxo2'); if (!ctx || !window.Chart) return;
    destroyChart('chartFluxo2');

    const meses = rangeMeses(R.periodo);
    const tx = filtrarTx();

    const somaPorMes = {};
    meses.forEach(m => somaPorMes[m]=0);
    tx.forEach(t=>{
      const m = String(t.data).slice(0,7);
      const s = (t.tipo==='Despesa') ? -Number(t.valor||0) : Number(t.valor||0);
      somaPorMes[m] = (somaPorMes[m]||0) + s;
    });
    ...
    R.charts.chartFluxo2 = new Chart(ctx, {
      type: 'bar',
      data: { labels, datasets: [{ label: 'Fluxo (R$)', data: meses.map(m=>somaPorMes[m]) }] },
      options:{ responsive:true, maintainAspectRatio:false }
    });
  }

  function renderCategorias(){
    const ctx = qs('#chartPie2'); if (!ctx || !window.Chart) return;
    destroyChart('chartPie2');

    // Força análise de despesas para este painel
    const backupTipo = R.tipo;
    R.tipo = 'Despesa';
    const tx = filtrarTx();
    R.tipo = backupTipo;
    ...
    const valores = labels.map(k => porCat[k]);
    R.charts.chartPie2 = new Chart(ctx, {
      type: 'pie',
    ...
    // Tabela Top
    const tbody = qs('#tblTop2 tbody'); if (tbody){
      tbody.innerHTML = '';
      labels
        .map(cat => ({ cat, total: porCat[cat] }))
        .sort((a,b)=>b.total-a.total)
        .forEach(row=>{
          const tr = document.createElement('tr');
          tr.innerHTML = `<td>${row.cat||'-'}</td><td>${brl(row.total)}</td>`;
          tbody.appendChild(tr);
        });
    }
  }

  function regressaoLinear(pontos){ // retorna {a,b} para y = a*x + b
    // x = 0..n-1
    const n = pontos.length;
    if (n === 0) return {a:0,b:0};
    let sx=0, sy=0, sxy=0, sxx=0;
    for(let i=0;i<n;i++){ const x=i, y=pontos[i]; sx+=x; sy+=y; sxy+=x*y; sxx+=x*x; }
  ...
  function renderPrevisao(){
    const ctx = qs('#chartForecast2'); if (!ctx || !window.Chart) return;
    destroyChart('chartForecast2');

    const meses = rangeMeses(R.periodo);
    const tx = filtrarTx();

    // Saldo mensal (receitas - despesas)
    const saldo = meses.map(m=>{
      const doMes = tx.filter(t => String(t.data).slice(0,7)===m);
      const rec = doMes.filter(t=>t.tipo==='Receita').reduce((a,b)=>a+Number(b.valor||0),0);
      const des = doMes.filter(t=>t.tipo==='Despesa').reduce((a,b)=>a+Number(b.valor||0),0);
      return rec - des;
    });

    // Regressão e projeção +3 meses
    const {a,b} = regressaoLinear(saldo);
    const projMeses = 3;
    const forecast = [];
    for(let i=0;i<saldo.length+projMeses;i++){
      forecast.push(a*i + b);
    }
    ...
    const labelsF = labels.concat(['+1','+2','+3']);

    R.charts.chartForecast2 = new Chart(ctx, {
      type: 'line',
      data: {
        labels: labelsF,
        datasets: [
          { label: 'Saldo real', data: saldo, borderWidth:2, tension:.25 },
          { label: 'Tendência/Previsão', data: forecast, borderDash:[6,6], borderWidth:2, tension:.25 }
        ]
      },
      options:{ responsive:true, maintainAspectRatio:false }
    });

    const final = forecast[forecast.length-1] || 0;
    const kpi = qs('#kpiForecastFinal2');
    if (kpi){ kpi.textContent = brl(final); kpi.classList.toggle('blurred', !!(S && S.hide)); }
    // Média por categoria no período
    const tbody = qs('#tblMediaCats2 tbody'); if (tbody){
      const despesas = filtrarTx().filter(t=>t.tipo==='Despesa');
      const agg = {};
      despesas.forEach(t=>{ agg[t.categoria] = (agg[t.categoria]||0) + Number(t.valor||0); });
      const mesesN = Math.max(1, rangeMeses(R.periodo).length);
      tbody.innerHTML = '';
      Object.keys(agg).sort((a,b)=>agg[b]-agg[a]).forEach(cat=>{
        const tr = document.createElement('tr');
        tr.innerHTML = `<td>${cat||'-'}</td><td>${brl(agg[cat]/mesesN)}</td>`;
        tbody.appendChild(tr);
      });
    }
  }
  ...
  function renderHeatmap(){
    const wrap = qs('#heatmap2'); if (!wrap) return;
    wrap.innerHTML = '';
    const curYM = thisMonthYM();
    const tx = (S.tx||[]).filter(t=>t.data && String(t.data).startsWith(curYM) && t.tipo==='Despesa');
    const porDia = {};
    tx.forEach(t=>{
      const d = new Date(t.data);
      const key = d.getDate();
      porDia[key] = (porDia[key]||0) + Number(t.valor||0);
    });
    const max = Math.max(1, ...Object.values(porDia).concat(0));
    const daysInMonth = new Date(+curYM.slice(0,4), +curYM.slice(5,7), 0).getDate();

    // grade simples 7 col x ~5 linhas
    ...
    for(let day=1; day<=daysInMonth; day++){
      const v = porDia[day]||0;
      const inten = v/max; // 0..1
      const cell = document.createElement('div');
      cell.style.height = '24px';
      cell.style.borderRadius = '6px';
      cell.style.border = '1px solid var(--border)';
      cell.style.background = `rgba(239,68,68, ${0.12 + 0.55*inten})`;
      cell.title = `${String(day).padStart(2,'0')}/${curYM.slice(5,7)}: ${brl(v)}`;
  ...
  function renderYoY(){
    const ctx = qs('#chartYoY'); if (!ctx || !window.Chart) return;
    destroyChart('chartYoY');

    const now = new Date((thisMonthYM()+'-01').replace(/-/g,'/'));
    const y = now.getFullYear(), prev = y-1;

    function somaPorMesAno(ano){
      const out = Array.from({length:12}, _=>0);
      (S.tx||[]).forEach(t=>{
        if (!t.data) return;
        const d = new Date(t.data);
    ...
    const cur = somaPorMesAno(y);
    const old = somaPorMesAno(prev);
    const labels = Array.from({length:12},(_,i)=>new Date(2000,i,1).toLocaleDateString('pt-BR',{month:'short'}));

    R.charts.chartYoY = new Chart(ctx, {
      type: 'line',
      data: {
        labels,
        datasets: [
          { label: `${prev}`, data: old, borderWidth:2, tension:.25 },
          { label: `${y}`,    data: cur, borderWidth:2, tension:.25 }
        ]
      },
      options:{ responsive:true, maintainAspectRatio:false }
    });
  ...
  function renderRxV(){
    const ctx = qs('#chartRxV'); if (!ctx || !window.Chart) return;
    destroyChart('chartRxV');

    const meses = rangeMeses(R.periodo);
    const tx = filtrarTx();

    const r = {}, d = {};
    meses.forEach(m=>{ r[m]=0; d[m]=0; });
    tx.forEach(t=>{
      const m = String(t.data).slice(0,7);
      if (t.tipo==='Receita') r[m]+=Number(t.valor||0);
      if (t.tipo==='Despesa') d[m]+=Number(t.valor||0);
    });
    ...
    R.charts.chartRxV = new Chart(ctx, {
      type: 'bar',
      data: {
        labels,
        datasets: [
          { label: 'Receitas', data: meses.map(m=>r[m]) },
          { label: 'Despesas', data: meses.map(m=>d[m]) }
        ]
      },
      options: {
  ...
  // ---------- Export (PNG / CSV do período) ----------
  function exportPNG(which){
    const map = {
      fluxo:    '#chartFluxo2',
      categorias: '#chartPie2',
      previsao: '#chartForecast2',
      heatmap:  null,
      yoy:      '#chartYoY',
      rxv:      '#chartRxV'
    };
    ...
    if (!c) return;
    const link = document.createElement('a');
    link.download = `relatorio-${which}.png`;
    link.href = c.toDataURL('image/png');
    link.click();
  }
  ...
  function exportCSV(){
    const tx = filtrarTx();
    const rows = [
      ['data','tipo','categoria','carteira','forma_pagamento','descricao','valor']
    ].concat(tx.map(t=>[
      t.data, t.tipo, t.categoria, t.carteira||'', t.pagamento||t.forma_pagamento||'',
      (t.descricao||t.desc||'').replace(/\r?\n/g,' '), String(t.valor||0).replace('.',',')
    ]));
    const csv = rows.map(r=>r.map(x=>`"${String(x).replace(/"/g,'""')}"`).join(';')).join('\n');
    const blob = new Blob([csv], {type:'text/csv;charset=utf-8;'});
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = 'relatorios-export.csv'; a.click();
    URL.revokeObjectURL(url);
  }

  // ---------- Tela cheia ----------
  function enableFullscreen(which){
    const map = {
      fluxo: '#chartFluxo2',
      pie: '#chartPie2',
    ...
    const src = qs(map[which]); if (!src) return;
    const fs = qs('#chartFs'); const dst = qs('#chartFsCanvas'); if (!fs || !dst) return;
    fs.hidden = false;
    // Clona dataset para o canvas de FS
    const cfg = R.charts[src.id]?.config;
    if (!cfg) return;
    destroyChart('chartFsCanvas');
    R.charts.chartFsCanvas = new Chart(dst.getContext('2d'), JSON.parse(JSON.stringify(cfg)));
  }

  function wireFullscreenAndExports(){
    document.addEventListener('click', (e)=>{
      const btn = e.target.closest('[data-fs]');
      if (btn) { enableFullscreen(btn.getAttribute('data-fs')); }
      const exp = e.target.closest('[data-export]');
      if (exp) { exportPNG(exp.getAttribute('data-export')); }
    });
    const close = qs('#fsClose'), fs = qs('#chartFs');
    if (close && fs) close.addEventListener('click', ()=>{ fs.hidden = true; destroyChart('chartFsCanvas'); });
  }

  // ---------- Render do painel ativo ----------
  function renderActive(){
    if (R.tab==='fluxo') renderFluxo();
    else if (R.tab==='categorias') renderCategorias();
    else if (R.tab==='previsao') renderPrevisao();
    else if (R.tab==='heatmap') renderHeatmap();
    else if (R.tab==='yoy') renderYoY();
    else if (R.tab==='rxv') renderRxV();
  }

  // ---------- Inicialização pública ----------
  window.initReportsUI = function initReportsUI(){
    // Subtabs
    qsa('.rtab').forEach(btn=>{
      btn.addEventListener('click', ()=>{
        R.tab = btn.dataset.rtab;
        qsa('.rtab').forEach(b=>b.classList.toggle('active', b===btn));
        qsa('.rpanel').forEach(p=>p.classList.toggle('active', p.dataset.rtab===R.tab));
        renderActive();
      });
    });

    // Filtros
    const period = qs('#rPeriodo'), tipo = qs('#rTipo'), cat = qs('#rCategoria');
    if (period) period.addEventListener('change', ()=>{ R.periodo = period.value; renderActive(); });
    if (tipo)   tipo.addEventListener('change',   ()=>{ R.tipo    = tipo.value;   renderActive(); });
    if (cat)    cat.addEventListener('change',    ()=>{ R.categoria = cat.value;  renderActive(); });

    // Popular categorias (únicas) no select
    if (cat){
      const cats = Array.from(new Set((S.tx||[]).map(t=>t.categoria).filter(Boolean))).sort();
      cat.innerHTML = `<option value="todas" selected>Todas</option>` + cats.map(c=>`<option value="${c}">${c}</option>`).join('');
    }

    // Exportar (CSV do período/recorte atual)
    const btnExport = qs('#rExport');
    if (btnExport) btnExport.addEventListener('click', exportCSV);

    // Tela cheia & export PNG por painel
    wireFullscreenAndExports();

    // Primeira renderização
    renderActive();
  };

  // Opcional: se já existe um ciclo de render principal, chamar após os dados carregarem
  document.addEventListener('DOMContentLoaded', ()=>{
    try { initReportsUI(); } catch(_) {}
  });
