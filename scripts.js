...
(function(){
  // Safe Chart holder
  let CH = { carteiras: null, pagamentos: null };

  function getTxForReports(){
    try {
      // Prefer a helper if app defines it
      if (typeof txSelected === 'function') return txSelected();
    } catch(_){}
    // Fallback: filter by active range/month
    try {
      const ym = (window.S && S.month) ? S.month : null;
      if (!ym) return (S.tx||[]);
      if (typeof getActiveRangeForYM === 'function') {
        const r = getActiveRangeForYM(ym);
        return (S.tx||[]).filter(t => t && t.data && ymdInRange(String(t.data), r.start, r.end));
      }
    } catch(_){}
    return (S.tx||[]);
  }

  function sumByCarteira(list){
    const out = { Casa:0, Marido:0, Esposa:0 };
    list.forEach(t => {
      if (!t || t.tipo!=="Despesa") return;
      const w = t.carteira || "Casa";
      if (out[w] == null) out[w] = 0;
      out[w] += Number(t.valor)||0;
    });
    return out;
  }

  function sumByFormaPagamento(list){
    const out = { dinheiro:0, pix:0, cartao:0, outros:0 };
    list.forEach(t => {
      if (!t || t.tipo!=="Despesa") return;
      var f = (t.forma_pagamento!=null) ? t.forma_pagamento : null;
      try { f = normalizeFormaPagamento(f); } catch(_){ f = 'outros'; }
      if (out[f] == null) out[f] = 0;
      out[f] += Number(t.valor)||0;
    });
    return out;
  }

  function toPct(n, total){
    const v = total>0 ? (100 * (Number(n)||0) / total) : 0;
    return (Math.round(v*10)/10).toFixed(1) + "%";
  }

  function ensureChart(ctxId, kind){
    const ctx = document.getElementById(ctxId);
    if (!ctx) return null;
    if (CH[kind]) { CH[kind].destroy(); CH[kind]=null; }
    // eslint-disable-next-line no-undef
    return new Chart(ctx, {
      type: (kind==='carteiras' ? 'pie' : 'bar'),
      data: { labels: [], datasets: [{ data: [] }] },
      options: {
        responsive: true,
        maintainAspectRatio: false,
        plugins: { legend: { position: 'bottom' } },
        scales: (kind==='pagamentos' ? { y: { beginAtZero:true } } : {})
      }
    });
  }

  function renderCarteirasPagamentos(){
    const list = getTxForReports();
    const byW  = sumByCarteira(list);
    const byF  = sumByFormaPagamento(list);

    // Charts
    CH.carteiras = ensureChart('chartCarteiras', 'carteiras');
    CH.pagamentos = ensureChart('chartPagamentos', 'pagamentos');

    if (CH.carteiras){
      const labels = Object.keys(byW);
      const values = labels.map(k => byW[k]||0);
      CH.carteiras.data.labels = labels;
      CH.carteiras.data.datasets[0].data = values;
      CH.carteiras.update();
    }
    if (CH.pagamentos){
      const labels = Object.keys(byF).map(k => (typeof humanFormaPagamento==='function'?humanFormaPagamento(k):k));
      const values = ['dinheiro','pix','cartao','outros'].map(k => byF[k]||0);
      CH.pagamentos.data.labels = labels;
      CH.pagamentos.data.datasets[0].data = values;
      CH.pagamentos.update();
    }

    // Table
    const tbody = document.querySelector('#tblCarteirasPagamentos tbody');
    if (tbody){
      const totalW = Object.values(byW).reduce((a,b)=>a+(Number(b)||0),0);
      const totalF = Object.values(byF).reduce((a,b)=>a+(Number(b)||0),0);
      const rows = [];
      Object.keys(byW).forEach(k => {
        rows.push(`<tr><td>Carteira</td><td>${k}</td><td>${(Number(byW[k])||0).toLocaleString('pt-BR',{style:'currency',currency:'BRL'})}</td><td>${toPct(byW[k], totalW)}</td></tr>`);
      });
      [['dinheiro','Dinheiro'],['pix','Pix'],['cartao','Cartão'],['outros','Outros']].forEach(([k,label])=>{
        rows.push(`<tr><td>Pagamento</td><td>${label}</td><td>${(Number(byF[k])||0).toLocaleString('pt-BR',{style:'currency',currency:'BRL'})}</td><td>${toPct(byF[k], totalF)}</td></tr>`);
      });
      tbody.innerHTML = rows.join('');
    }
  }

  // Hook: when switching to this rtab, render it
  document.addEventListener('click', function(ev){
    const btn = ev.target.closest && ev.target.closest('.rtab[data-rtab="carteiraspgto"]');
    if (btn) setTimeout(renderCarteirasPagamentos, 0);
  });

  // Also re-render when filters change
  ['rPeriodo','rTipo','rCategoria','monthSelect'].forEach(id=>{
    const el = document.getElementById(id);
    if (el && !el._wiredCarteirasPagamentos){
      el.addEventListener('change', ()=>{
        const panel = document.querySelector('.rpanel[data-rtab="carteiraspgto"]');
        if (panel && panel.classList.contains('active')) renderCarteirasPagamentos();
      });
      el._wiredCarteirasPagamentos = true;
    }
  });

  // Expose for manual trigger after loadAll/render if needed
  try { window.renderCarteirasPagamentos = renderCarteirasPagamentos; } catch(_){}
})();
