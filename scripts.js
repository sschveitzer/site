...
(function(){
  ...
  if (typeof window.Utils === "undefined") {
    window.Utils = {
      ...
      money(v){ return (typeof v === 'number') ? v : (String(v||'').length? Number(String(v).replace(/\./g,"").replace(",","."))||0 : 0); },
      addDays(ymd, days){
      ...
      incMonthly(ymd, diaMes, ajusteFimMes = true){
        ...
        const ld = this.lastDayOfMonth(yy, mm);
        const day = ajusteFimMes ? Math.min(diaMes, ld) : diaMes;
        return this.toYMD(new Date(yy, mm - 1, day));
      },
      incWeekly(ymd){ return this.addDays(ymd, 7); },
      incYearly(ymd, diaMes, mes, ajusteFimMes = true){
        const [y] = ymd.split("-").map(Number);
        const yy = y + 1;
        const ld = this.lastDayOfMonth(yy, mes);
        const day = ajusteFimMes ? Math.min(diaMes, ld) : diaMes;
        return this.toYMD(new Date(yy, mes - 1, day));
      },
  ...
  if (typeof window.UI === "undefined") {
    window.UI = {
      ...
      upsertTxInMemory(t){
        try {
          const idx = (window.S && Array.isArray(window.S.tx)) ? window.S.tx.findIndex(x=>x.id===t.id) : -1;
          if (idx>=0) window.S.tx[idx] = t; else if (window.S) window.S.tx.push(t);
        } catch {}
      },
      removeTxInMemory(id){
        try { if (window.S) window.S.tx = (window.S.tx||[]).filter(x=>x.id!==id); } catch {}
      },
      rerender(){
