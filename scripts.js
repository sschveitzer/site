...
// ================================
// ASSISTENTE FINANCEIRO - VERSÃO 5 (LIMPO)
// ================================

if (typeof ultimaCategoriaPerguntada === "undefined") {
  var ultimaCategoriaPerguntada = null;
}
...
function responderPergunta(texto) {
  ...
  if (q.includes("previsao") || q.includes("previsão")) {
    ...
    if (cat) return `No ritmo atual, você vai gastar ${fmtMoneyLocal(preverCategoria(cat,mes))} em ${cat}.`;
    return "Diga a categoria para prever (ex: previsão mercado).";
  }
  ...
  if (q.includes("mais que mês passado") || q.includes("comparar")) {
    ...
    if (diff > 0) return `Você gastou ${fmtMoneyLocal(diff)} a mais que no mês passado.`;
    if (diff < 0) return `Você economizou ${fmtMoneyLocal(Math.abs(diff))} em relação ao mês passado.`;
  ...
  return "Exemplos: 'resumo do mês', 'onde mais gastei', 'limpar chat', 'previsão mercado'.";
}
