...
/* ==== PATCH: Recorrências desativadas de forma segura (sem remover nada) ====
   Este bloco sobrescreve as funções de recorrência como NO-OPs e garante que
   o checkbox de repetir (#mRepetir) seja ignorado, preservando o resto do app.
   Caso precise reverter, remova este bloco inteiro.
------------------------------------------------------------------------------- */
(function () {
  try { window.REC_DISABLED = true; console.log('[patch] recorrencias desativadas'); } catch (_) {}

  // --- Utilidades seguras ---
  function _safeSet(obj, prop, val) { try { obj[prop] = val; } catch (_) {} }
  function _noopPromise() { return Promise.resolve({ data: null, error: null }); }

  // --- Stubs / NO-OPs para API de recorrências ---
  // Mantém as mesmas assinaturas públicas para compatibilidade.
  try {
    // Se existirem em escopo global, sobrescrevemos por funções no-op
    window.saveRec = async function saveRec() { return _noopPromise(); };
    window.deleteRec = async function deleteRec() { return _noopPromise(); };
    window.toggleRecAtivo = async function toggleRecAtivo() { return _noopPromise(); };
    window.materializeOne = async function materializeOne() { return; };
    window.applyRecurrences = async function applyRecurrences() { return; };
  } catch (_) {
    // Ambiente sem window
  }

  // --- Blindagem em caso de escopos locais (sem window) ---
  // Define no escopo global (quando possível) nomes que o resto do código possa chamar.
  if (typeof saveRec === "function") {
    try { saveRec = window.saveRec; } catch (_) {}
  }
  if (typeof deleteRec === "function") {
    try { deleteRec = window.deleteRec; } catch (_) {}
  }
  if (typeof toggleRecAtivo === "function") {
    try { toggleRecAtivo = window.toggleRecAtivo; } catch (_) {}
  }
  if (typeof materializeOne === "function") {
    try { materializeOne = window.materializeOne; } catch (_) {}
  }
  if (typeof applyRecurrences === "function") {
    try { applyRecurrences = window.applyRecurrences; } catch (_) {}
  }

  // --- Força S.recs = [] após carregamentos ---
  // Se existir objeto S, garantimos que recs fique vazio para não confundir a UI.
  try {
    if (typeof S === "object" && S) { S.recs = []; }
  } catch (_) {}

  // --- Se existir função loadAll, envelopa para garantir S.recs = [] ao final ---
  try {
    if (typeof window !== "undefined" && typeof window.loadAll === "function") {
      const __oldLoadAll = window.loadAll;
      window.loadAll = async function wrappedLoadAll() {
        const res = await __oldLoadAll.apply(this, arguments);
        try { if (typeof S === "object" && S) { S.recs = []; } } catch (_) {}
        return res;
      };
    }
  } catch (_) {}

  // --- Impede criação de recorrência via checkbox (#mRepetir) ---
  // 1) Desmarca no carregamento
  function _disableRecurrenceCheckbox() {
    try {
      const el = document.querySelector("#mRepetir");
      if (el && el.checked) el.checked = false;
      // Evita que volte a marcar
      if (el && !el.__recDisabled) {
        el.__recDisabled = true;
  ...
  if (typeof document !== "undefined") {
    if (document.readyState === "loading") {
      document.addEventListener("DOMContentLoaded", _disableRecurrenceCheckbox);
    } else {
      _disableRecurrenceCheckbox();
    }
    // Também observa re-renderizações dinâmicas
    try {
      const mo = new MutationObserver(_disableRecurrenceCheckbox);
      mo.observe(document.documentElement || document.body, { childList: true, subtree: true });
    } catch (_) {}
  }

  // --- Envelopa addOrUpdate (se existir) para garantir caminho "não recorrente" ---
  // Isso garante que, independentemente da lógica interna, o checkbox estará falso
  // no momento do submit, caindo no fluxo de saveTx normal.
  try {
    if (typeof window !== "undefined" && typeof window.addOrUpdate === "function") {
      const __oldAddOrUpdate = window.addOrUpdate;
      window.addOrUpdate = async function wrappedAddOrUpdate() {
        try {
          const chk = (typeof document !== "undefined") ? document.querySelector("#mRepetir") : null;
          if (chk && chk.checked) chk.checked = false; // força modo não recorrente
        } catch (_) {}
        return await __oldAddOrUpdate.apply(this, arguments);
      };
    }
  } catch (_) {}

  // --- Segurança extra: se algum código tentar usar applyRecurrences diretamente ---
  try { if (typeof applyRecurrences === "function") { applyRecurrences = async () => {}; } } catch (_) {}

})();
/* ==== FIM DO PATCH DE RECORRÊNCIAS DESATIVADAS ==== */
